...
function App() {
  ...
  // useEffect(() => {
  //   csv("/unemployment-by-county-2017.csv").then(data => setData(data));
  // }, []);
    ...
    <div style={{ width: '90vw', margin: '0 auto' }}>
      ...
      <ComposableMap projection="geoAlbersUsa">
        <Geographies geography={geoUrl}>
          {({ geographies }) =>
            geographies.map(geo => {
              const cur = data.find(s => Number(s.fips) === Number(geo.id));

              return (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
                  fill={cur ? colorScale(cur.bachelorsOrHigher) : '#FFF'}
                  className="county"
                  data-fips={cur?.fips}
                  data-education={cur?.bachelorsOrHigher}
                />
              );
            })
          }
